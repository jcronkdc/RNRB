    ...
    if (!type || type === "all" || type === "project") {
      ...
      results.push(
        ...matchingProjects.map(
          (p: { id: string; name: string; description?: string | null; slug: string }) => ({
            id: p.id,
      ...
      for (const project of projects.projects.slice(0, 10)) {
        ...
        try {
          ...
          const matchingSongs = songs.filter(
            (s: { title: string; key?: string | null }) =>
          ...
          results.push(
            ...matchingSongs.map((s: { id: string; title: string }) => ({
              ...
              href: `/projects/${project.slug}?tab=songs`,
            })),
          );
        } catch {
    ...
    if (!type || type === "all" || type === "asset") {
      ...
      for (const project of projects.projects.slice(0, 10)) {
        try {
          ...
          const matchingAssets = assets.filter((a: { name: string }) =>
          ...
          results.push(
            ...matchingAssets.map((a: { id: string; name: string }) => ({
              ...
              title: a.name,
              subtitle: `In ${project.name}`,
              href: `/projects/${project.slug}?tab=assets`,
            })),
          );
        } catch {
          // Skip if error
