...
// .wrangler/tmp/bundle-bucoyl/strip-cf-connecting-ip-header.js
...
var CreateSupportTicketSchema = external_exports.object({
  subject: external_exports.string().min(5).describe("A clear, concise subject line for the ticket"),
  description: external_exports.string().min(20).describe("Detailed description of the issue"),
...
var ViewTicketsSchema = external_exports.object({
  status: external_exports.array(external_exports.enum(["OPEN", "IN_PROGRESS", "WAITING_ON_USER", "RESOLVED", "CLOSED"])).optional().describe("Filter by status"),
  limit: external_exports.number().min(1).max(50).optional().describe("Max tickets to return")
});
var ViewTicketDetailsSchema = external_exports.object({
  ticketNumber: external_exports.string().describe("Ticket number (e.g., RNRB-1234) or ticket ID")
});
var ReplyToTicketSchema = external_exports.object({
  ticketNumber: external_exports.string().describe("Ticket number (e.g., RNRB-1234)"),
  message: external_exports.string().min(1).describe("Message to add to the ticket")
});
var SubscribeNewsletterSchema = external_exports.object({
  firstName: external_exports.string().optional().describe("First name for personalization"),
  preferences: external_exports.object({
    productUpdates: external_exports.boolean().optional(),
    tips: external_exports.boolean().optional(),
    events: external_exports.boolean().optional(),
    community: external_exports.boolean().optional()
  }).optional().describe("Email preferences"),
  frequency: external_exports.enum(["REALTIME", "DAILY", "WEEKLY", "MONTHLY"]).optional().describe("How often to receive emails")
});
var UnsubscribeNewsletterSchema = external_exports.object({
  reason: external_exports.string().optional().describe("Optional reason for unsubscribing")
});
var TroubleshootIssueSchema = external_exports.object({
  ...
  description: external_exports.string().describe("Description of the problem"),
  browserInfo: external_exports.string().optional().describe("Browser and version if known"),
  deviceInfo: external_exports.string().optional().describe("Device type if relevant")
});
var SendFeedbackSchema = external_exports.object({
  type: external_exports.enum(["FEEDBACK", "FEATURE_REQUEST", "PRAISE", "SUGGESTION"]).describe("Type of feedback"),
  message: external_exports.string().min(10).describe("The feedback message"),
  area: external_exports.string().optional().describe("Which area of the app this relates to")
});
var RNRB_TOOLS = [
  {
...
// src/handlers.ts
async function rnrbApi(env2, endpoint, options = {}) {
  const { method = "GET", body, userContext } = options;
  const headers = {
    "Content-Type": "application/json",
    "X-MCP-Server-Key": env2.RNRB_API_KEY
  };
  if (userContext?.accessToken) {
    headers["Authorization"] = `Bearer ${userContext.accessToken}`;
  }
  try {
    const response = await fetch(`${env2.RNRB_API_URL}${endpoint}`, {
      method,
      headers,
    ...
    console.error(`[MCP] API error for ${endpoint}:`, error3);
    return { success: false, error: "Failed to connect to RNRB API" };
  }
}
__name(rnrbApi, "rnrbApi");
async function handleCreateSupportTicket(env2, userContext, args) {
  ...
  const result = await rnrbApi(env2, "/api/support/tickets", {
    method: "POST",
    body: {
...
__name(handleCreateSupportTicket, "handleCreateSupportTicket");
async function handleViewTickets(env2, userContext, args) {
  ...
  const result = await rnrbApi(env2, `/api/support/tickets?${params}`, { userContext });
  if (!result.success) {
    return {
      content: [
        {
  ...
  const ticketList = data.tickets.map(
  ...
  return {
    content: [
      {
        type: "text",
        text: `\u{1F4CB} **Your Support Tickets** (${data.total} total)
...
__name(handleViewTickets, "handleViewTickets");
async function handleViewTicketDetails(env2, userContext, args) {
  ...
  const result = await rnrbApi(env2, `/api/support/tickets/${parsed.data.ticketNumber}`, {
    userContext
  });
  if (!result.success) {
    return {
  ...
  const messages = ticket.messages.map(
    (m) => `**${m.senderType === "USER" ? "You" : m.senderName || "Support"}** (${new Date(m.createdAt).toLocaleString()}):
${m.content}`
  ).join("\n\n---\n\n");
  return {
    content: [
      {
        type: "text",
        text: `\u{1F3AB} **Ticket ${ticket.ticketNumber}**
...
__name(handleViewTicketDetails, "handleViewTicketDetails");
async function handleReplyToTicket(env2, userContext, args) {
...
__name(handleReplyToTicket, "handleReplyToTicket");
async function handleSubscribeNewsletter(env2, userContext, args) {
  ...
  const result = await rnrbApi(env2, "/api/newsletter/subscribe", {
    method: "POST",
    body: {
      email: userContext.email,
      ...parsed.data,
  ...
  return {
    content: [
      {
        type: "text",
        text: `\u{1F4E7} ${data.message}${data.requiresConfirmation ? "\n\nCheck your email to confirm your subscription!" : ""}`
      }
    ]
  };
}
__name(handleSubscribeNewsletter, "handleSubscribeNewsletter");
async function handleUnsubscribeNewsletter(env2, userContext, args) {
  ...
  const result = await rnrbApi(env2, "/api/newsletter/unsubscribe", {
    method: "POST",
    body: {
      email: userContext.email,
...
__name(handleUnsubscribeNewsletter, "handleUnsubscribeNewsletter");
async function handleTroubleshootIssue(_env, _userContext, args) {
  ...
  return {
    content: [
      {
        type: "text",
        text: `\u{1F527} **Troubleshooting: ${parsed.data.issueType} Issue**
...
__name(handleTroubleshootIssue, "handleTroubleshootIssue");
async function handleCheckSystemStatus(_env, _userContext, _args) {
  const services = [
  ...
  return {
    content: [
      {
        type: "text",
        text: `\u{1F7E2} **RNRB System Status**

All systems operational.

${statusList}

_Last checked: ${(/* @__PURE__ */ new Date()).toLocaleString()}_`
      }
    ]
  };
}
__name(handleCheckSystemStatus, "handleCheckSystemStatus");
async function handleSendFeedback(env2, userContext, args) {
  ...
  const responseText = parsed.data.type === "FEATURE_REQUEST" ? "Our team reviews all feature requests and uses them to guide our roadmap." : "Your feedback helps us make Rock N' Roll Basement better for everyone.";
  return {
    content: [
      {
        type: "text",
...
__name(handleSendFeedback, "handleSendFeedback");
...
app.get("/sse", async (c) => {
  ...
  const sendEvent = /* @__PURE__ */ __name(async (event, data) => {
...
`));
  ...
  (async () => {
    try {
      ...
      console.error("[MCP] SSE error:", error3);
    }
  })();
  ...
  try {
    switch (name) {
      case "create_support_ticket":
        result = await handleCreateSupportTicket(env2, userContext, args);
        break;
      case "view_my_tickets":
        result = await handleViewTickets(env2, userContext, args);
        break;
      case "view_ticket_details":
        result = await handleViewTicketDetails(env2, userContext, args);
        break;
      case "reply_to_ticket":
        result = await handleReplyToTicket(env2, userContext, args);
        break;
      case "subscribe_newsletter":
        result = await handleSubscribeNewsletter(env2, userContext, args);
        break;
      case "unsubscribe_newsletter":
        result = await handleUnsubscribeNewsletter(env2, userContext, args);
        break;
      case "troubleshoot_issue":
        result = await handleTroubleshootIssue(env2, userContext, args);
        break;
      case "check_system_status":
        result = await handleCheckSystemStatus(env2, userContext, args);
        break;
      case "send_feedback":
        result = await handleSendFeedback(env2, userContext, args);
        break;
      default:
    ...
    console.error(`[MCP] Tool error for ${name}:`, error3);
    return c.json(
      {
    ...
    case "tools/call": {
      ...
      try {
        switch (toolParams.name) {
          case "create_support_ticket":
            result = await handleCreateSupportTicket(env2, userContext, toolParams.arguments);
            break;
          case "view_my_tickets":
            result = await handleViewTickets(env2, userContext, toolParams.arguments);
            break;
          case "view_ticket_details":
            result = await handleViewTicketDetails(env2, userContext, toolParams.arguments);
            break;
          case "reply_to_ticket":
            result = await handleReplyToTicket(env2, userContext, toolParams.arguments);
            break;
          case "subscribe_newsletter":
            result = await handleSubscribeNewsletter(env2, userContext, toolParams.arguments);
            break;
          case "unsubscribe_newsletter":
            result = await handleUnsubscribeNewsletter(env2, userContext, toolParams.arguments);
            break;
          case "troubleshoot_issue":
            result = await handleTroubleshootIssue(env2, userContext, toolParams.arguments);
            break;
          case "check_system_status":
            result = await handleCheckSystemStatus(env2, userContext, toolParams.arguments);
            break;
          case "send_feedback":
            result = await handleSendFeedback(env2, userContext, toolParams.arguments);
            break;
          default:
            return c.json(
              {
        ...
        console.error(`[MCP] Tool error:`, error3);
        return c.json(
          {
            jsonrpc: "2.0",
            id,
            error: { code: -32e3, message: "Tool execution failed" }
          },
          500
        );
      }
...
// .wrangler/tmp/bundle-bucoyl/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-bucoyl/middleware-loader.entry.ts
