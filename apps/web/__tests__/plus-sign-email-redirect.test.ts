...
describe('Email Plus Sign Redirect Flow', () => {
  ...
  it('should preserve plus sign through invite → auth → profile → accept flow', () => {
    ...
    // Step 4: User completes profile setup
    // Profile page decodes redirect param
    const decodedRedirect = decodeURIComponent(returnUrl);
    console.log('Step 4 - Decoded redirect:', decodedRedirect);
    // Expected: /invites/test-project?email=user+test@example.com

    // Step 5: Profile page re-encodes query params before navigation
    const url = new URL(decodedRedirect, 'http://dummy.com');
    ...
    // Step 6: Invites page receives navigation, parses email
    const finalUrlObj = new URL(finalUrl, 'http://dummy.com');
  ...
  it('should handle multiple special characters in email', () => {
    ...
    const inviteUrl = `/invites/${projectSlug}?email=${encodeURIComponent(specialEmail)}`;
    
    // Decode as profile setup would
    const returnUrl = `/invites/${projectSlug}?email=${encodeURIComponent(specialEmail)}`;
    const decodedRedirect = decodeURIComponent(returnUrl);
    
    // Re-encode as profile setup fix does
    const url = new URL(decodedRedirect, 'http://dummy.com');
    const params = new URLSearchParams();
    url.searchParams.forEach((value, key) => {
      params.set(key, value);
  ...
  it('should handle edge cases', () => {
    ...
    const url = new URL(decodedMulti, 'http://dummy.com');
    const params = new URLSearchParams();
    url.searchParams.forEach((value, key) => {
      params.set(key, value);
    });
