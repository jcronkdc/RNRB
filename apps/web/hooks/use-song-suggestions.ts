...
export function useSongSuggestions({
  ...
  const acceptChordSuggestion = useCallback((suggestionId: string) => {
    if (!channelRef.current || !isOwner) return;
  ...
  const rejectChordSuggestion = useCallback((suggestionId: string) => {
    if (!channelRef.current || !isOwner) return;
  ...
  const getSuggestionsForBlock = useCallback((blockId: string) => {
    return Array.from(suggestions.values()).filter(s => s.blockId === blockId);
  }, [suggestions]);

  const getChordSuggestionsForBlock = useCallback((blockId: string) => {
