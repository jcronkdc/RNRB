  ...
  return (
    <div className="rounded-xl border border-(--border) bg-(--panel) p-6">
      <div className="mb-4 flex items-start justify-between">
        ...
        {trend && (
          <TrendingUp
            className={`h-5 w-5 ${trend === 'up' ? 'text-(--sage)' : 'rotate-180 text-(--error)'}`}
          />
        )}
      </div>
      <div className="mb-1 text-2xl font-bold text-(--text)">{value}</div>
      <div className="text-sm text-(--muted)">{label}</div>
      {subtext && <div className="mt-1 text-xs text-(--accent)">{subtext}</div>}
    </div>
  );
}
    ...
    <Link href={`/masterclasses/instructor/${masterclass.id}/edit`}>
      <div className="flex items-center gap-4 rounded-xl border border-(--border) bg-(--panel) p-4 transition-all hover:border-(--accent)/50">
        ...
        <div className="min-w-0 flex-1">
          <h4 className="truncate font-medium text-(--text)">{masterclass.title}</h4>
      ...
      <div className="min-h-screen bg-(--bg)">
        <div className="mx-auto max-w-4xl px-4 py-16">
          ...
          <div className="mb-12 text-center">
            <div className="mb-6 inline-flex h-20 w-20 items-center justify-center rounded-full bg-linear-to-br from-(--accent) to-(--gold)">
          ...
          <div className="mb-12 grid grid-cols-1 gap-6 md:grid-cols-3">
            <div className="rounded-xl border border-(--border) bg-(--panel) p-6 text-center">
              <div className="mb-2 text-3xl font-bold text-(--accent)">70%</div>
              <div className="font-medium text-(--text)">Revenue Share</div>
              <div className="mt-1 text-sm text-(--muted)">You keep 70% of all earnings</div>
            </div>
            <div className="rounded-xl border border-(--border) bg-(--panel) p-6 text-center">
              <div className="mb-2 text-3xl font-bold text-(--accent)">Unlimited</div>
              <div className="font-medium text-(--text)">Students</div>
              <div className="mt-1 text-sm text-(--muted)">Reach musicians globally</div>
            </div>
            <div className="rounded-xl border border-(--border) bg-(--panel) p-6 text-center">
              <div className="mb-2 text-3xl font-bold text-(--accent)">Live + VOD</div>
              <div className="font-medium text-(--text)">Flexible Format</div>
              <div className="mt-1 text-sm text-(--muted)">Live sessions or pre-recorded</div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-(--accent)/20 bg-linear-to-r from-(--accent)/15 to-(--gold)/15 p-8">
            ...
            <Link href="/masterclasses/instructor/setup">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-linear-to-r from-(--accent) to-(--gold) py-4 font-semibold text-white"
              >
      ...
      <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-(--text)">{instructor.displayName}</h1>
              <p className="text-(--muted)">{instructor.headline || 'Instructor'}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Link href="/masterclasses/instructor/settings">
              <button className="rounded-xl border border-(--border) bg-(--panel) p-3 text-(--muted) transition-colors hover:text-(--text)">
            ...
            <Link href="/masterclasses/instructor/new">
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="flex items-center gap-2 rounded-xl bg-linear-to-r from-(--accent) to-(--gold) px-4 py-3 font-medium text-white"
              >
        ...
        {!stripeStatus?.onboarded && (
          ...
          >
            ...
            <div className="flex-1">
              <h3 className="font-medium text-(--warning)">Set Up Payouts</h3>
            ...
            <motion.button
              ...
              className="flex items-center gap-2 rounded-lg bg-(--warning) px-4 py-2 font-medium text-black"
            >
              {connectingStripe ? (
        ...
        <div className="rounded-xl border border-(--border) bg-(--panel)">
          <div className="flex items-center justify-between border-b border-(--border) p-6">
            <h2 className="text-xl font-bold text-(--text)">Your Masterclasses</h2>
            <Link
              ...
              <div className="py-12 text-center">
                ...
                <Link href="/masterclasses/instructor/new">
                  <motion.button
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className="inline-flex items-center gap-2 rounded-xl bg-linear-to-r from-(--accent) to-(--gold) px-6 py-3 font-medium text-white"
                  >
                    <Plus className="h-5 w-5" />
