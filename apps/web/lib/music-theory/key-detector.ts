...
export function detectKey(chords: string[]): KeySuggestion[] {
  ...
  const parsedChords = chords.map(parseChord).filter((c): c is ParsedChord => c !== null);
  ...
  const allNotes = Object.keys(NOTE_TO_NUMBER).filter(
    (n) => !n.includes('#') || n === 'F#' || n === 'C#'
  );
  ...
  for (const chord of chords) {
    ...
    if (fitsInKey) {
      ...
      const diatonicMatch = diatonicChords.find(
        (dc) =>
          NOTE_TO_NUMBER[dc.root] === NOTE_TO_NUMBER[chord.root] && dc.quality === chord.quality
      );
