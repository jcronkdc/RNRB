...
export default function InviteModal({ isOpen, onClose, orgId, projectId, onInviteSent, type }: InviteModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              disabled={isSending || success}
              className="h-12 w-full rounded-2xl border border-border bg-surface px-4 text-base focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
            >
          ...
          {error && (
            <motion.div
              ...
              className="rounded-2xl border border-danger/60 bg-danger/10 px-4 py-3 text-sm text-danger-foreground"
            >
              {error}
            </motion.div>
          )}
          ...
          {success && (
            <motion.div
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
              className="rounded-2xl border border-success/60 bg-success/10 px-4 py-3 text-sm text-success-foreground"
            >
        ...
        <div className="mt-6 rounded-2xl border border-border/60 bg-muted/50 p-4 text-xs text-muted-foreground">
          <p>
            <strong>Note:</strong> Invitations expire after 7 days. The recipient must have a CronkWaters account
