...
function SortableChordBlock({
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className="rnrb-card group border-brand-primary/30 from-brand-primary/10 to-brand-primary/5 hover:border-brand-primary/50 relative cursor-move border-2 bg-linear-to-br p-6 transition-all duration-300 hover:shadow-2xl"
    >
      {/* Grip Indicator */}
      <div className="absolute top-1/2 left-2 -translate-y-1/2 opacity-30 transition group-hover:opacity-60">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex h-8 w-8 items-center justify-center rounded-lg bg-red-500/10 text-red-500 opacity-0 transition group-hover:opacity-100 hover:bg-red-500/20"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
  ...
  return (
    <div ref={setNodeRef} style={style} className="group relative inline-flex">
      <div
        {...attributes}
        {...listeners}
        className="font-display border-brand-primary/40 from-brand-primary/20 to-brand-primary/10 text-brand-primary hover:border-brand-primary/60 relative cursor-move rounded-xl border-2 bg-linear-to-br px-4 py-3 text-lg font-bold transition-all duration-200 hover:scale-105 hover:shadow-lg"
      >
        <GripVertical className="absolute top-1/2 left-1 h-3 w-3 -translate-y-1/2 opacity-0 transition group-hover:opacity-40" />
      ...
      <button
        ...
        className="absolute -top-2 -right-2 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-white opacity-0 shadow-lg transition group-hover:opacity-100 hover:bg-red-600"
      >
        <X className="h-3 w-3" />
      </button>
    </div>
...
export function ChordBuilder({ onChange }: { onChange: (chords: ChordBlock[]) => void }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Chord Palette */}
      {showChordPalette && (
        <Card className="rnrb-card border-brand-primary/20 from-brand-primary/5 border-2 bg-linear-to-br to-transparent p-8 shadow-2xl">
      ...
      <Card className="rnrb-card from-surface to-surface-muted/50 min-h-[400px] bg-linear-to-b p-8">
        {chords.length === 0 ? (
      ...
      {chords.length > 0 && (
        <>
          <div className="rnrb-card to-brand-primary/5 border-2 border-green-500/20 bg-linear-to-r from-green-500/5 p-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="font-display text-foreground text-2xl leading-relaxed font-bold">
          ...
          {analysis && analysis.mostLikelyKey && (
            <div className="rnrb-card space-y-4 border-2 border-purple-500/20 bg-linear-to-r from-purple-500/5 to-blue-500/5 p-6">
              <div className="flex items-start gap-4">
